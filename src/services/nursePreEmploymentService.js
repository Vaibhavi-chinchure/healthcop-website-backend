...
export const addClient = async (clientData) => {
  ...
  if (!client_name?.trim()) throw new Error("Client name is required");
  if (total_sites === undefined || isNaN(total_sites)) throw new Error("Total sites must be a valid number");
  if (total_sites < 0) throw new Error("Total sites cannot be negative");

  for (const site of sites) {
    if (!site.site_name?.trim()) throw new Error("Site name is required");
    if (!Array.isArray(site.deliverables)) throw new Error("Deliverables must be an array");
  ...
  try {
    ...
    if (lastRecord.length > 0 && lastRecord[0].client_id) {
      const lastNumber = parseInt(lastRecord[0].client_id.split("-")[2], 10);
      if (!isNaN(lastNumber)) newIdNumber = lastNumber + 1;
    }
    const newClientId = `HOP-CLI-${newIdNumber.toString().padStart(4, "0")}`;
    ...
    if (clientResult.affectedRows === 0) throw new Error("Failed to insert Client");
    ...
    for (const site of sites) {
      ...
      if (siteResult.affectedRows === 0) throw new Error(`Failed to insert site ${site.site_name}`);
    }
    ...
    for (const officer of officers) {
      if (officer.name && officer.position && officer.phone && officer.email) {
        ...
        if (officerResult.affectedRows === 0) throw new Error(`Failed to insert officer ${officer.name}`);
      }
    }
...
export const updateClient = async (id, clientData) => {
  ...
  try {
    ...
    if (clientResult.affectedRows === 0) throw new Error("No Client found to update");
...
export const deleteClient = async (id) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) throw new Error("No Client found to delete");
