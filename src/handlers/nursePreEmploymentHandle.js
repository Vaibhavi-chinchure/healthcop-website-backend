...
//   try {
//     const siteId = req.cookies?.siteId || null;
//     const userRole = req.user?.role || 'admin'; // Default to admin if not set

//     let query = queries.GET_ALL_LABORERS;
//     let params = [];

//     // Filter by site_id for non-admin roles
...
// Helper: Safely parse JSON or wrap in array
const safeParseJSON = (value) => {
  if (!value) return [];
  try {
    return JSON.parse(value);
...
const addPreEmployment = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < laborersData.length; i++) {
      const laborer = laborersData[i];
      if (!laborer.name || !laborer.name.trim()) continue;
